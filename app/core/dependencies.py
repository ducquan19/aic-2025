...
def get_model_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Model service initialization failed: {str(e)}"
        )
...
def get_keyframe_service(
    ...
    try:
        ...
        if keyframe_service is None:
            ...
            raise HTTPException(
                status_code=503, detail="Keyframe service not available"
            )
        return keyframe_service
    except Exception as e:
        logger.error(f"Failed to get keyframe service: {str(e)}")
        raise HTTPException(
            status_code=503, detail=f"Keyframe service initialization failed: {str(e)}"
        )
...
def get_milvus_repository(
    ...
    try:
        ...
        if repository is None:
            raise HTTPException(
                status_code=503, detail="Milvus repository not available"
            )
        return repository
    except Exception as e:
        logger.error(f"Failed to get Milvus repository: {str(e)}")
        raise HTTPException(
            status_code=503, detail=f"Milvus repository initialization failed: {str(e)}"
        )
...
def get_query_controller(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Query controller initialization failed: {str(e)}"
        )
